...
| Vercel WalletConnect configuration                     | Blocked                       | No Vercel environment variables were present when inspected on 2026-07-21                            |
| Vercel preview build                                   | Passed                        | `https://arc-crosschain-checkout-5c9058xm4.vercel.app`; build completed with missing-env diagnostics |
| Anonymous deployed-browser check                       | Blocked by protection         | Preview redirects unauthenticated Chromium to Vercel login; local Chromium coverage passed           |
...
Passing mock tests must not be reported as proof of WalletConnect relay, QR, mobile deep-link, extension interoperability, or real signing. Add dated browser/device, wallet name/version, deployed URL, and observed result here after following [WALLETCONNECT_SETUP.md](WALLETCONNECT_SETUP.md). Never record seed phrases, private keys, session topics, or the full project ID.
